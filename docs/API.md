...
`GET /servers/:id/export?backups=true&events=true&start=true` sends the download as `<server-name>.cbx` — the server name with anything outside `A-Z a-z 0-9 _ -` replaced by `_`, so `Test Paper` exports as `Test_Paper.cbx` (every download named after a server does the same) — with `Content-Type: application/x-craftbox-export+zip` and an exact `Content-Length`. The server must be `stopped` or `crashed` (`409` otherwise). Query flags (`true` to enable): `backups` and `events` select the optional payloads; `start` starts the server once the archive has finished streaming (used by the panel's "Start server after export" option) — an abandoned download leaves the server stopped. Requesting `backups` holds the backup lock while the archive is packed; `409` if a backup is already running. `507` if the staging area cannot hold the archive.
