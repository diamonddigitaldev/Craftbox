...
A **text search that spans several loaders** (modpacks with no `loader`, or a plugin platform, whose family is Paper/Spigot/Bukkit) is unioned server-side rather than handed to Modrinth as one OR'd facet. Modrinth decides how far to relax the query terms from whatever the facet leaves it, so the same query answers differently per loader and the OR'd facet can drop a loader's results entirely — searching modpacks for `optimised fps`, `loader=forge` returns 13 hits led by a 577k-download pack that `All Loaders` did not list at all. Craftbox therefore runs the OR'd search **and** one search per loader, then merges and de-duplicates them, so the result contains every loader's best hits. The merge reads the top 100 of each list, so a multi-loader text search pages through up to a few hundred results; single-loader and query-less searches are a single request and page as deep as Modrinth allows.
